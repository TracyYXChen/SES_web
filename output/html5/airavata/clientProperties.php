...
if ( null === $appjson ) {
    echo "The file /opt/genapp/ses/appconfig.json could not be decoded and is likely missing or corrupt\n";
    exit;
}
if ( !array_key_exists( 'resources', $appjson ) ) {
    echo "No resources defined in /opt/genapp/ses/appconfig.json. Fix this issue and try again.\n";
    exit;
}
if ( !array_key_exists( 'airavata', $appjson->resources ) ) {
    echo "resources:airavata must be defined in /opt/genapp/ses/appconfig.json. Fix this issue and try again.\n";
    exit;
}
if ( !array_key_exists( 'properties', $appjson->resources->airavata ) ) {
    echo "resources:airavata:properties must be defined in /opt/genapp/ses/appconfig.json. Fix this issue and try again.\n";
    exit;
}
if ( !array_key_exists( 'resources', $appjson->resources->airavata ) ) {
    echo "resources:airavata:resources must be defined in /opt/genapp/ses/appconfig.json. Fix this issue and try again.\n";
    exit;
}

$properties = $appjson->resources->airavata->properties;

$GLOBALS["GENAPP_APPLICATION"]               = "ses";
