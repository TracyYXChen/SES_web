...
$cmd = isset( $_REQUEST[ '_docrootexecutable' ] ) ? "/var/www/html/ses/" . $_REQUEST[ '_docrootexecutable' ] : "/opt/genapp/ses/bin/sys_manage_users";
...
function last_nonnumeric( $a ) {
    ...
    if ( $i < 0 ) {
        error_log( "ses admin sys_manage_users last_nonnumeric could not find one\n" . json_encode( $a, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
        return $a[ 0 ];
    }
                ...
                if ( 1 ) {
                    ...
                    if ( count( $b ) ) {
                        ...
                        if ( !is_array( $_REQUEST[ $tag ] ) ) {
                            error_log( "ses admin sys_manage_users target tag $tag in not an array in request v $v\n" . json_encode( $_REQUEST, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                        } else {
                            $obj = &$_REQUEST[ $tag ];
                            foreach ( $b as $v2 ) {
                                if ( !isset( $obj[ $v2 ] ) ) {
                                ...
                                if ( !is_array( $obj[ $v2 ] ) ) {
                                    error_log( "ses admin sys_manage_users target tag $tag in not an array in request v $v object\n" . json_encode( $obj, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                                    break;
                                }
                                if ( count( $obj ) <= $v2 ) {
