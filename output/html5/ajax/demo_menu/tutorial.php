...
if ( !sizeof( $_REQUEST ) )
{
    ...
    error_mail( $msg . "Error occured in demo_menu tutorial.\n" );
...
$cmd = isset( $_REQUEST[ '_docrootexecutable' ] ) ? "/var/www/html/rotdif/" . $_REQUEST[ '_docrootexecutable' ] : "/opt/genapp/rotdif/bin/__executable__";
...
$GLOBALS[ 'module'    ] = "tutorial";
$GLOBALS[ 'jobweight' ] = floatval( "__jobweight__" );
$GLOBALS[ 'menu'      ] = "demo_menu";
...
if ( sizeof( $_FILES ) ) {
    ...
    $module_json = json_decode( '{"fields":[{"id":"text1","role":"input","type":"html","value":"        <div>             <p> Hello, this is the test file             </p>             <p>                 <a name=\"_GoBack\"> </a> <img style=\"width:75%\" src=\"files/tutorial_tutorial_files/rId4.png\" title=\"\" alt=\"\" align=\"\" /></p> </div> "}],"label":"ROTDIF tutorial","moduleid":"tutorial","nooutputhr":"true","noreset":"true","nosubmit":"true"}' );
    $required_files = [];
...
function last_nonnumeric( $a ) {
    ...
    if ( $i < 0 ) {
        error_log( "rotdif demo_menu tutorial last_nonnumeric could not find one\n" . json_encode( $a, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
        return $a[ 0 ];
    }
...
if ( sizeof( $_REQUEST ) )
{
                ...
                if ( 1 ) {
                    ...
                    if ( count( $b ) ) {
                        ...
                        if ( !is_array( $_REQUEST[ $tag ] ) ) {
                            error_log( "rotdif demo_menu tutorial target tag $tag in not an array in request v $v\n" . json_encode( $_REQUEST, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                        } else {
                            $obj = &$_REQUEST[ $tag ];
                            foreach ( $b as $v2 ) {
                                if ( !isset( $obj[ $v2 ] ) ) {
                                ...
                                if ( !is_array( $obj[ $v2 ] ) ) {
                                    error_log( "rotdif demo_menu tutorial target tag $tag in not an array in request v $v object\n" . json_encode( $obj, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                                    break;
                                }
                                if ( count( $obj ) <= $v2 ) {
    ...
    if ( isset( $fileargs ) )
    {
      ...
      if( $cmdprefix == "airavatarun" ||
          $cmdprefix == "oscluster" ) { 
         $cmd = "$adir/$cmdprefix";
         $cmd .= $cmdprefix == "oscluster" ? " __executable__" : " tutorial";
         $cmd .= " '$json'"; 
      } else {
          if ( strlen( $cmdprefix ) ) {
