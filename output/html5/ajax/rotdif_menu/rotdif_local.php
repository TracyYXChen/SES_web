...
if ( !sizeof( $_REQUEST ) )
{
    ...
    error_mail( $msg . "Error occured in rotdif_menu rotdif_local.\n" );
...
$cmd = isset( $_REQUEST[ '_docrootexecutable' ] ) ? "/var/www/html/rotdif/" . $_REQUEST[ '_docrootexecutable' ] : "/opt/genapp/rotdif/bin/rotdif_full.py";
...
$adir = "/var/www/html/rotdif";

if ( !isset( $uniquedir ) &&
     isset( $_SESSION[ $window ][ 'logon' ] ) &&
     strlen( $_SESSION[ $window ][ 'logon' ] ) > 1 )
{
   $dir = "/var/www/html/rotdif/results/users/" . $_SESSION[ $window ][ 'logon' ] . "/";
   $bdir = $dir;
...
// connect
   ...
   do {
       $dir = uniqid( "/var/www/html/rotdif/results/" );
...
$app_json = json_decode( file_get_contents( "/opt/genapp/rotdif/appconfig.json" ) );
...
if ( !sizeof( $_FILES ) ) {
   ...
   foreach ( $_REQUEST as $k=>$v ) {
      if ( !strncmp( $k, $selalt, $lenselalt ) ) {
          ...
          error_log( "rotdif_full.py no files but found _selaltval_ with key $tmp_key\n", 3, "/tmp/mylog_selalt" );
      }
   }
   if ( $found_selalt ) {
       error_log( "rotdif_full.py request\n" . print_r( $_REQUEST, true ) . "\n", 3, "/tmp/mylog_selalt" );
       error_log( "files\n" . print_r( $_FILES, true ) . "\n", 3, "/tmp/mylog_selalt" );
   }
}
...
if ( sizeof( $_FILES ) ) {
    ...
    $module_json = json_decode( '{"executable":"rotdif_full.py","fields":[{"help":"Enter a project name","id":"projectname","label":"Project name","required":"true","role":"input","specifiedproject":"Run_","type":"text"},{"default":"","id":"data_label","label":"DATA","posthline":"true","prehline":"true","role":"input","type":"label"},{"accept":".txt","help":"Upload relaxation file","id":"relax_location","label":"Relaxation File","required":"true","role":"input","type":"lrfile"},{"accept":".pdb","help":"Upload PDB file","id":"pdb_location","label":"PDB File","required":"true","role":"input","type":"lrfile"},{"default":"","id":"runmenu_label","label":"RUN MENU","posthline":"true","prehline":"true","role":"input","type":"label"},{"default":"leastsq","help":"Choose optimization method","id":"optimization_method","label":"Optimization Method","repeater":"yes","role":"input","type":"listbox","values":"Least Squares~leastsq~Robust Least Squares (3.0sig)~robust"},{"checked":"false","id":"stat","label":"Full Statistics ","role":"input","type":"checkbox"},{"id":"progress_output","label":"Progress: ","max":1,"role":"output","type":"progress"},{"cols":40,"id":"progress_text","label":"Calculation Report: ","role":"output","type":"textarea"},{"id":"outputrotdif","label":"Detailed Results File: ","multiple":"true","role":"output","type":"file"},{"id":"axi_out","label":"Save PyMol Axes: axially symmetrical","multiple":"true","role":"output","type":"file"},{"id":"ani_out","label":"Save PyMol Axes: anisotropic","multiple":"true","role":"output","type":"file"},{"id":"exp_plot","label":"Experiment Data","role":"output","showcollapse":"true","type":"plot3d"},{"height":"450px","id":"vec_plot","label":"Bond Vector Plot","role":"output","showcollapse":"true","type":"plotly","width":"700px"},{"height":"450px","id":"chi2_plot","label":"Chi-square Plot","role":"output","showcollapse":"true","type":"plotly","width":"700px"},{"backgroundcolor":"white","changescalex":"true","changescaley":"true","customtooltips":"true","height":"450px","hover":"true","id":"fit_plot","label":"2D Model Fit:","pan":"false","role":"output","rotatedylabel":"true","savetofile":"true","selzoom":"true","showcollapse":"true","type":"plot2d","width":"700px","zoom":"false"},{"id":"plot_3d","label":"3D Model Fit:","role":"output","showcollapse":"true","type":"plot3d"},{"id":"iso_plot","label":"Isotropic Model:","role":"output","showcollapse":"true","type":"plotly"},{"id":"axi_plot","label":"Axially Symmetric Model:","role":"output","showcollapse":"true","type":"plotly"},{"id":"ani_plot","label":"Fully Anisotropic Model:","role":"output","showcollapse":"true","type":"plotly"},{"id":"diso_plot","label":"Isotropic Model Dynamics Plot:","role":"output","showcollapse":"true","type":"plotly"},{"id":"daxi_plot","label":"Axially Symmetric Model Dynamics Plot:","role":"output","showcollapse":"true","type":"plotly"},{"id":"dani_plot","label":"Fully Anisotropic Model Dynamics Plot:","role":"output","showcollapse":"true","type":"plotly"}],"label":"Calculate","moduleid":"rotdif_local","notify":"email","submitpolicy":"login"}' );
    $required_files = [];
...
function last_nonnumeric( $a ) {
    ...
    if ( $i < 0 ) {
        error_log( "rotdif rotdif_menu rotdif_local last_nonnumeric could not find one\n" . json_encode( $a, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
        return $a[ 0 ];
    }
...
if ( sizeof( $_REQUEST ) )
{
                ...
                if ( 1 ) {
                    ...
                    if ( count( $b ) ) {
                        ...
                        if ( !is_array( $_REQUEST[ $tag ] ) ) {
                            error_log( "rotdif rotdif_menu rotdif_local target tag $tag in not an array in request v $v\n" . json_encode( $_REQUEST, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                        } else {
                            $obj = &$_REQUEST[ $tag ];
                            foreach ( $b as $v2 ) {
                                if ( !isset( $obj[ $v2 ] ) ) {
                                ...
                                if ( !is_array( $obj[ $v2 ] ) ) {
                                    error_log( "rotdif rotdif_menu rotdif_local target tag $tag in not an array in request v $v object\n" . json_encode( $obj, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                                    break;
                                }
                                if ( count( $obj ) <= $v2 ) {
    ...
    if ( isset( $fileargs ) )
    {
      ...
      if( $cmdprefix == "airavatarun" ||
          $cmdprefix == "oscluster" ) { 
         $cmd = "$adir/$cmdprefix";
         $cmd .= $cmdprefix == "oscluster" ? " rotdif_full.py" : " rotdif_local";
         $cmd .= " '$json'"; 
      } else {
          if ( strlen( $cmdprefix ) ) {
              $register = "perl $adir/util/ga_regpid_udp.pl rotdif " . 
                  $GLOBALS['resource'] . " " . 
                  $_REQUEST[ '_udphost' ] . " " .
                  $_REQUEST[ '_udpport' ] . " " .
                  $_REQUEST[ '_uuid' ] . " " .
    ...
    $altcmd = "nohup /usr/local/bin/php /var/www/html/rotdif/util/jobrun.php '" . $GLOBALS[ 'logon' ] . "' " . $_REQUEST[ '_uuid' ] . " " . ( isset( $checkrunning ) ? "1" : "0" ) . " 2>&1 >> /tmp/php_errors &";
...
//    error_log( "\taltcmd:\n$altcmd\n", 3, "/tmp/mylog" );
    ...
    $results = str_replace( "/var/www/html/rotdif/", "", $results );
    if ( $do_logoff == 1 )
    {   
        $results = substr( trim( $results ), 0, -1 ) + ",\"_logon\":\"\"}";
    }
