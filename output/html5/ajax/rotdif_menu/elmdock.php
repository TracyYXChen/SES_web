...
if ( !sizeof( $_REQUEST ) )
{
    ...
    error_mail( $msg . "Error occured in rotdif_menu elmdock.\n" );
...
$cmd = isset( $_REQUEST[ '_docrootexecutable' ] ) ? "/var/www/html/ses/" . $_REQUEST[ '_docrootexecutable' ] : "/opt/genapp/ses/bin/main_elmdock.py";
     ...
     !isset( $useresource ) )
{
   ...
   if ( strlen( $cmdprefix ) > 1 ) {
      ...
      $cmdprefix = str_replace( "_" . "_menu:modules:id__", "elmdock", $cmdprefix );
...
if ( !sizeof( $_FILES ) ) {
   ...
   foreach ( $_REQUEST as $k=>$v ) {
      if ( !strncmp( $k, $selalt, $lenselalt ) ) {
          ...
          error_log( "main_elmdock.py no files but found _selaltval_ with key $tmp_key\n", 3, "/tmp/mylog_selalt" );
      }
   }
   if ( $found_selalt ) {
       error_log( "main_elmdock.py request\n" . print_r( $_REQUEST, true ) . "\n", 3, "/tmp/mylog_selalt" );
       error_log( "files\n" . print_r( $_FILES, true ) . "\n", 3, "/tmp/mylog_selalt" );
   }
}
...
if ( sizeof( $_FILES ) ) {
    ...
    $module_json = json_decode( '{"executable":"main_elmdock.py","fields":[{"default":"header4","id":"ELMDOCK_label","label":"Diffusion Tensor-Guided Docking (ELMDOCK)","posthline":"true","prehline":"true","role":"input","type":"label"},{"help":"Enter a name to run this task","id":"run_name","label":"run name","required":"true","role":"input","type":"text"},{"default":"","id":"data_label","label":"DATA (ONLY available for 2-domain proteins)","posthline":"true","prehline":"true","role":"input","type":"label"},{"accept":".txt","help":"Upload relaxation file","id":"relax_location","label":"Relaxation Data File","required":"False","role":"input","type":"lrfile"},{"accept":".pdb","help":"Upload PDB file","id":"pdb_location","label":"PDB Coordinate File","required":"true","role":"input","type":"lrfile"},{"default":"1","id":"model","label":"PDB File model","required":"True","role":"input","type":"text"},{"default":"","id":"runmenu_label","label":"RUN MENU","posthline":"true","prehline":"true","role":"input","type":"label"},{"default":"leastsq","help":"Choose optimization method","id":"optimization_method","label":"Optimization Method","repeater":"yes","role":"input","type":"listbox","values":"Least Squares~leastsq~Robust Least Squares (3.0sig)~robust"},{"default":"298","id":"temperature","label":"Temperature(K)","required":"True","role":"input","type":"text"},{"checked":"false","id":"adv_option","label":"check box to enable advanced model","repeater":"true","role":"input","type":"checkbox"},{"default":"2.2","id":"hydro","label":"Hydration layer thickness(Angstrom)","repeat":"adv_option","required":"True","role":"input","type":"text"},{"default":"1.4","id":"water","label":"Water radius(Angstrom)","repeat":"adv_option","required":"True","role":"input","type":"text"},{"id":"progress_output","label":"Progress: ","max":1,"role":"output","type":"progress"},{"cols":40,"id":"progress_text","label":"Calculation Report: ","role":"output","type":"textarea"},{"id":"outputrotdif","label":"Detailed Results File: ","multiple":"true","role":"output","type":"file"},{"id":"elmdock_out","label":"ELMDOCK results &nbsp;&nbsp;","multiple":"true","role":"output","type":"file"},{"id":"pdb","label":"ELMDOCK PDB file ","multiple":"true","role":"output","type":"file"},{"height":"850","id":"outputpdb","label":"Docking View","role":"output","type":"atomicstructure","width":"850"}],"label":"Calculate","moduleid":"elmdock","notify":"email","submitpolicy":"login"}' );
    $required_files = [];
...
function last_nonnumeric( $a ) {
    ...
    if ( $i < 0 ) {
        error_log( "ses rotdif_menu elmdock last_nonnumeric could not find one\n" . json_encode( $a, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
        return $a[ 0 ];
    }
                ...
                if ( 1 ) {
                    ...
                    if ( count( $b ) ) {
                        ...
                        if ( !is_array( $_REQUEST[ $tag ] ) ) {
                            error_log( "ses rotdif_menu elmdock target tag $tag in not an array in request v $v\n" . json_encode( $_REQUEST, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                        } else {
                            $obj = &$_REQUEST[ $tag ];
                            foreach ( $b as $v2 ) {
                                if ( !isset( $obj[ $v2 ] ) ) {
                                ...
                                if ( !is_array( $obj[ $v2 ] ) ) {
                                    error_log( "ses rotdif_menu elmdock target tag $tag in not an array in request v $v object\n" . json_encode( $obj, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                                    break;
                                }
                                if ( count( $obj ) <= $v2 ) {
    ...
    if ( isset( $fileargs ) )
    {
      ...
      if ( $cmdprefix == "airavatarun" ||
          ...
          if ( substr( $cmdprefix, 0, 5 ) != "abaco" ) {
              $cmd .= $cmdprefix == "oscluster" ? " main_elmdock.py" : " elmdock";
          }
          $cmd .= " '$json'"; 
      } else {
