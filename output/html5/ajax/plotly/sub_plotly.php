...
if ( !sizeof( $_REQUEST ) )
{
    ...
    error_mail( $msg . "Error occured in plotly sub_plotly.\n" );
...
if ( sizeof( $_FILES ) ) {
    ...
    $module_json = json_decode( '{"executable":"test_subplot.py","fields":[{"id":"subplot","label":"Example subplot","role":"output","type":"plotly"}],"help":"help for Plotly","label":"plotly plot","moduleid":"plotly","uniqueid":"on"}' );
    $required_files = [];
...
function last_nonnumeric( $a ) {
    ...
    if ( $i < 0 ) {
        error_log( "rotdif plotly sub_plotly last_nonnumeric could not find one\n" . json_encode( $a, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
        return $a[ 0 ];
    }
                ...
                if ( 1 ) {
                    ...
                    if ( count( $b ) ) {
                        ...
                        if ( !is_array( $_REQUEST[ $tag ] ) ) {
                            error_log( "rotdif plotly sub_plotly target tag $tag in not an array in request v $v\n" . json_encode( $_REQUEST, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                        } else {
                            $obj = &$_REQUEST[ $tag ];
                            foreach ( $b as $v2 ) {
                                if ( !isset( $obj[ $v2 ] ) ) {
                                ...
                                if ( !is_array( $obj[ $v2 ] ) ) {
                                    error_log( "rotdif plotly sub_plotly target tag $tag in not an array in request v $v object\n" . json_encode( $obj, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                                    break;
                                }
                                if ( count( $obj ) <= $v2 ) {
