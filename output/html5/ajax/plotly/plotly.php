...
if ( !sizeof( $_REQUEST ) )
{
    ...
    error_mail( $msg . "Error occured in plotly plotly.\n" );
...
$cmd = isset( $_REQUEST[ '_docrootexecutable' ] ) ? "/var/www/html/rotdif/" . $_REQUEST[ '_docrootexecutable' ] : "/opt/genapp/rotdif/bin/plotly.py";
...
if ( !sizeof( $_FILES ) ) {
   ...
   foreach ( $_REQUEST as $k=>$v ) {
      if ( !strncmp( $k, $selalt, $lenselalt ) ) {
          ...
          error_log( "plotly.py no files but found _selaltval_ with key $tmp_key\n", 3, "/tmp/mylog_selalt" );
      }
   }
   if ( $found_selalt ) {
       error_log( "plotly.py request\n" . print_r( $_REQUEST, true ) . "\n", 3, "/tmp/mylog_selalt" );
       error_log( "files\n" . print_r( $_FILES, true ) . "\n", 3, "/tmp/mylog_selalt" );
   }
}
...
if ( sizeof( $_FILES ) ) {
    ...
    $module_json = json_decode( '{"executable":"plotly.py","fields":[{"id":"barchart","label":"Example Barchart","role":"output","type":"plotly"},{"id":"lineplot","label":"Example lineplot","role":"output","type":"plotly"}],"help":"help for Plotly","label":"Plotly","moduleid":"plotly","uniqueid":"on"}' );
    $required_files = [];
...
function last_nonnumeric( $a ) {
    ...
    if ( $i < 0 ) {
        error_log( "rotdif plotly plotly last_nonnumeric could not find one\n" . json_encode( $a, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
        return $a[ 0 ];
    }
...
if ( sizeof( $_REQUEST ) )
{
                ...
                if ( 1 ) {
                    ...
                    if ( count( $b ) ) {
                        ...
                        if ( !is_array( $_REQUEST[ $tag ] ) ) {
                            error_log( "rotdif plotly plotly target tag $tag in not an array in request v $v\n" . json_encode( $_REQUEST, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                        } else {
                            $obj = &$_REQUEST[ $tag ];
                            foreach ( $b as $v2 ) {
                                if ( !isset( $obj[ $v2 ] ) ) {
                                ...
                                if ( !is_array( $obj[ $v2 ] ) ) {
                                    error_log( "rotdif plotly plotly target tag $tag in not an array in request v $v object\n" . json_encode( $obj, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                                    break;
                                }
                                if ( count( $obj ) <= $v2 ) {
    ...
    if ( isset( $fileargs ) )
    {
      ...
      if( $cmdprefix == "airavatarun" ||
          $cmdprefix == "oscluster" ) { 
         $cmd = "$adir/$cmdprefix";
         $cmd .= $cmdprefix == "oscluster" ? " plotly.py" : " plotly";
         $cmd .= " '$json'"; 
      } else {
          if ( strlen( $cmdprefix ) ) {
