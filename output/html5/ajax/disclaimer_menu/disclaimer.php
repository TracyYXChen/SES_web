...
if ( !sizeof( $_REQUEST ) )
{
    ...
    error_mail( $msg . "Error occured in disclaimer_menu disclaimer.\n" );
...
$_REQUEST[ '_menu' ]        = "disclaimer_menu";
$_REQUEST[ '_module' ]      = "disclaimer";
     ...
     !isset( $useresource ) )
{
   ...
   if ( strlen( $cmdprefix ) > 1 ) {
      ...
      $cmdprefix = str_replace( "_" . "_menu:id__", "disclaimer_menu", $cmdprefix );
      $cmdprefix = str_replace( "_" . "_menu:modules:id__", "disclaimer", $cmdprefix );
...
function last_nonnumeric( $a ) {
    ...
    if ( $i < 0 ) {
        error_log( "ses disclaimer_menu disclaimer last_nonnumeric could not find one\n" . json_encode( $a, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
        return $a[ 0 ];
    }
                ...
                if ( 1 ) {
                    ...
                    if ( count( $b ) ) {
                        ...
                        if ( !is_array( $_REQUEST[ $tag ] ) ) {
                            error_log( "ses disclaimer_menu disclaimer target tag $tag in not an array in request v $v\n" . json_encode( $_REQUEST, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                        } else {
                            $obj = &$_REQUEST[ $tag ];
                            foreach ( $b as $v2 ) {
                                if ( !isset( $obj[ $v2 ] ) ) {
                                ...
                                if ( !is_array( $obj[ $v2 ] ) ) {
                                    error_log( "ses disclaimer_menu disclaimer target tag $tag in not an array in request v $v object\n" . json_encode( $obj, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                                    break;
                                }
                                if ( count( $obj ) <= $v2 ) {
    ...
    if ( isset( $fileargs ) )
    {
      ...
      if ( $cmdprefix == "airavatarun" ||
          ...
          if ( substr( $cmdprefix, 0, 5 ) != "abaco" ) {
              $cmd .= $cmdprefix == "oscluster" ? " __executable__" : " disclaimer";
          }
          $cmd .= " '$json'"; 
      } else {
