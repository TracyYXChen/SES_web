...
function get_userinfo( $error_json_exit = false ) {
   ...
   $runs = $use_db->ses->running->find();
   foreach ( $runs as $v ) {
       $uuid = $v['_id'];
       $job = $use_db->ses->jobs->findOne( array( "_id" => $uuid ), array( "user" => 1 ) );
       $name = $job[ 'user' ];
   ...
   $users = $use_db->ses->users->find();

   $users->sort( array( "name" => 1 ) );

   foreach ( $users as $v ) {
       $name = $v[ 'name' ];
       $jobcount = $use_db->ses->jobs->count( array( "user" => $name ) );
       if ( substr( $name, 0, strlen( "_canceled" ) ) != "_canceled" ) {
           ...
           if ( !isset( $v[ 'manageid' ] ) ) {
               ...
               try {
                   $use_db->ses->users->update( array( "_id" => new MongoId( $v[ "_id" ] ) ), 
                                  $update,
...
function handle_request() {
    ...
    if ( !$doc = $use_db->ses->users->findOne( [ "_id" => new MongoId( $_REQUEST[ '_id' ] ) ] ) ) {
        $results[ 'success' ] = "false";
    ...
    switch( $_REQUEST[ '_cmd' ] ) {
        case "remove" : {
            $mailuser = "Your account on http://" . $app->hostname . "/ses has been Removed.";
        ...
        case "approve" : {
            $mailuser = "Your request for an account on http://" . $app->hostname . "/ses has been Approved.";
            if ( isset( $doc[ 'needsemailverification' ] ) ) {
                $mailuser .= "\nPlease verify your email address by visiting this link:\n http://" . $app->hostname . "/ses/ajax/sys_config/sys_register_backend.php?_r=" . $doc[ '_id' ];
            } else {
                $mailuser .= "\nYou can now logon.";
            }
            $update = [ 
        ...
        case "deny" : {
            $mailuser = "Your request for an account on http://" . $app->hostname . "/ses has been Denied";
            $update = [
                '$unset' => [ 
                    "approvalid" => ""
                    ,"denyid" => "" 
        ...
        case "suspend" : {
            $mailuser = "Your account on http://" . $app->hostname . "/ses has been Suspended";
            $update = [ 
                '$set' => [
        ...
        case "activate" : {
            $mailuser = "Your account on http://" . $app->hostname . "/ses has been Activated";
            $update = [ 
                '$unset' => [ 
                    'suspended' => 1 
                ],
        ...
        case "resend_email_verification" : {
            ...
            $mailsubject = "[ses][email verify request]";
        }
        break;
    ...
    if ( isset( $update ) ) {
        ...
        try {
            $use_db->ses->users->update( [ "_id" => new MongoId( $_REQUEST[ '_id' ] ) ],
                                                     $update, array("j" => true ) );
        } catch(MongoCursorException $e) {
            $results[ 'success' ] = "false";
            $results[ 'error' ] = "Database error: $db_errors";
    ...
    if ( isset( $remove ) ) {
        try {
            $use_db->ses->users->remove( $remove, array("j" => true ) );
        } catch(MongoCursorException $e) {
            $results[ 'success' ] = "false";
            $results[ 'error' ] = "Database error: $db_errors";
            echo json_encode( $results );
        ...
        if ( isset( $removejobs ) ) {
            try {
                $use_db->ses->jobs->remove( [ "user" => $doc[ 'name' ] ], array("j" => true ) );
            } catch(MongoCursorException $e) {
                $results[ 'success' ] = "false";
                $results[ 'error' ] = "Database error: $db_errors";
                echo json_encode( $results );
        ...
        if ( isset( $removedata ) ) {
            $dir = "/var/www/html/ses/results/users/" . $doc[ 'name' ];
            if ( file_exists( $dir ) ) {
                $cstrong = true;
                $tmplen = 6;
                $uniq = bin2hex( openssl_random_pseudo_bytes ( $tmplen, $cstrong ) );
                $deldir = "/var/www/html/ses/removed/$uniq/" . $doc[ 'name' ];
                $tries = 0;
    ...
    if ( isset( $mailuser ) ) {
        mymail( $doc[ 'email' ], isset( $mailsubject ) ? $mailsubject : "[ses][account status update]", $mailuser );
    }
