...
if ( !sizeof( $_REQUEST ) )
{
    ...
    error_mail( $msg . "Error occured in ses_menu ses_all.\n" );
...
$cmd = isset( $_REQUEST[ '_docrootexecutable' ] ) ? "/var/www/html/ses/" . $_REQUEST[ '_docrootexecutable' ] : "/opt/genapp/ses/bin/main_ses.py";
...
$GLOBALS[ 'module'    ] = "ses_all";
$GLOBALS[ 'jobweight' ] = floatval( "__jobweight__" );
$GLOBALS[ 'menu'      ] = "ses_menu";
...
$GLOBALS[ 'notify' ] = "ychen151@terpmail.umd.edu";
$bdir = "";
...
$_REQUEST[ '_menu' ]        = "ses_menu";
$_REQUEST[ '_module' ]      = "ses_all";
     ...
     !isset( $useresource ) )
{
   ...
   if ( strlen( $cmdprefix ) > 1 ) {
      ...
      $cmdprefix = str_replace( "_" . "_menu:id__", "ses_menu", $cmdprefix );
      $cmdprefix = str_replace( "_" . "_menu:modules:id__", "ses_all", $cmdprefix );
...
if ( !sizeof( $_FILES ) ) {
   ...
   foreach ( $_REQUEST as $k=>$v ) {
      if ( !strncmp( $k, $selalt, $lenselalt ) ) {
          ...
          error_log( "main_ses.py no files but found _selaltval_ with key $tmp_key\n", 3, "/tmp/mylog_selalt" );
      }
   }
   if ( $found_selalt ) {
       error_log( "main_ses.py request\n" . print_r( $_REQUEST, true ) . "\n", 3, "/tmp/mylog_selalt" );
       error_log( "files\n" . print_r( $_FILES, true ) . "\n", 3, "/tmp/mylog_selalt" );
   }
}
...
function last_nonnumeric( $a ) {
    ...
    if ( $i < 0 ) {
        error_log( "ses ses_menu ses_all last_nonnumeric could not find one\n" . json_encode( $a, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
        return $a[ 0 ];
    }
                ...
                if ( 1 ) {
                    ...
                    if ( count( $b ) ) {
                        ...
                        if ( !is_array( $_REQUEST[ $tag ] ) ) {
                            error_log( "ses ses_menu ses_all target tag $tag in not an array in request v $v\n" . json_encode( $_REQUEST, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                        } else {
                            $obj = &$_REQUEST[ $tag ];
                            foreach ( $b as $v2 ) {
                                if ( !isset( $obj[ $v2 ] ) ) {
                                ...
                                if ( !is_array( $obj[ $v2 ] ) ) {
                                    error_log( "ses ses_menu ses_all target tag $tag in not an array in request v $v object\n" . json_encode( $obj, JSON_PRETTY_PRINT ) . "\n", 3, "/tmp/php_errors" );
                                    break;
                                }
                                if ( count( $obj ) <= $v2 ) {
    ...
    if ( isset( $fileargs ) )
    {
      ...
      if ( $cmdprefix == "airavatarun" ||
          ...
          if ( substr( $cmdprefix, 0, 5 ) != "abaco" ) {
              $cmd .= $cmdprefix == "oscluster" ? " main_ses.py" : " ses_all";
          }
          $cmd .= " '$json'"; 
      } else {
